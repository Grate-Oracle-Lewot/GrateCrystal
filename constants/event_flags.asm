...
; HMs (EVENT_GOT_HM07_WATERFALL is with the Johto itemballs)
	...
	const EVENT_GOT_TM_HYPER_BEAM_FROM_BLUE ; new to Grate Crystal
; Rods
...
; Johto story events
	...
	const EVENT_GOLDENROD_UNDERGROUND_MOVE_DELETER
	...
	const EVENT_GOT_TM_SANDSTORM_FROM_BROCK ; new to Grate Crystal
	const EVENT_GOT_TM_TELEPORT_FROM_SABRINA ; new to Grate Crystal
	...
	const EVENT_BEAT_TEACHER_CALYPSO ; new to Grate Crystal
...
; Johto hidden items
	...
	const EVENT_GOT_TM_BLIZZARD_FROM_MISTY ; new to Grate Crystal
	const EVENT_GOT_TM_THUNDER_FROM_SURGE ; new to Grate Crystal
	const EVENT_GOT_TM_FIRE_BLAST_FROM_BLAINE ; new to Grate Crystal
...
; new to Grate Crystal
	...
	const EVENT_DARKROOM_GAUNTLET_GOLD_TROPHY
	const EVENT_SEAFOAM_CAVE_HIEOUT_HIDDEN_TIMER_BALL
	const EVENT_SEAFOAM_CAVE_HIDEOUT_X_ACCURACY
	const EVENT_SEAFOAM_CAVE_HIDEOUT_HIDDEN_MAX_POTION
...
; Kanto hidden items
	...
	const EVENT_UNION_CAVE_HIDDEN_TIMER_BALL ; new to Grate Crystal
...
; new to Grate Crystal
	const EVENT_POKECENTER_2F_TRADEBACK_GUY
	const EVENT_GAVE_EON_MAIL_TO_GOLDENROD_POKEFAN_F
	const EVENT_ROUTE_44_DRAGONAIRS
	...
	const EVENT_DARKROOM_GAUNTLET
	const EVENT_DARKROOM_BEAT_LORELEI
	const EVENT_DARKROOM_BEAT_AGATHA
	const EVENT_DARKROOM_BEAT_BRUNO
	const EVENT_DARKROOM_BEAT_WILL
	const EVENT_BEAT_ENGINEER_SMITH
	const EVENT_BEAT_ENGINEER_BAILY
	...
	const EVENT_ROUTE39_BARN_MOOMOO_SICK
	const EVENT_ROUTE39_BARN_MOOMOO_HEALTHY
	const EVENT_OLIVINE_LIGHTHOUSE6F_AMPHY_SICK
	const EVENT_OLIVINE_LIGHTHOUSE6F_AMPHY_HEALTHY
	const EVENT_CERULEAN_CAVE_1F_SWAG_BEACON
	const EVENT_CERULEAN_CAVE_1F_X_SPECIAL
	const EVENT_CERULEAN_CAVE_1F_HIDDEN_LUCKY_PUNCH
	const EVENT_CERULEAN_CAVE_1F_ELECTRODE
	...
	const EVENT_CERULEAN_CAVE_2F_HIDDEN_HP_UP
	const EVENT_CERULEAN_CAVE_2F_HIDDEN_CARBOS
	const EVENT_CERULEAN_CAVE_2F_HIDDEN_MAX_REVIVE
	const EVENT_CERULEAN_CAVE_2F_HIDDEN_FULL_RESTORE
	const EVENT_CERULEAN_CAVE_2F_HIDDEN_STAR_PIECE
	const EVENT_CERULEAN_CAVE_2F_HIDDEN_NUGGET
	const EVENT_CERULEAN_CAVE_2F_HIDDEN_DUSK_BALL
	const EVENT_CERULEAN_CAVE_B1F_FEAROWBOT
	const EVENT_BATTLE_TOWER_INVERSE_MODE
	const EVENT_BATTLE_TOWER_TYPELESS_MODE
	const EVENT_BUGSY_ON_ROUTE_14
	const EVENT_OAKS_LAB_2F_BUGSY1
	const EVENT_OAKS_LAB_2F_BUGSY2
	const EVENT_OAKS_LAB_2F_BUG_CATCHER_ROB
	const EVENT_OAKS_LAB_2F_SCIENTIST_TAYLOR
	const EVENT_OAKS_LAB_2F_SCIENTIST_ELVIN
	const EVENT_OAKS_LAB_2F_FISHER_JONAH
	const EVENT_OAKS_LAB_2F_COOLTRAINERM_BLAKE
	const EVENT_OAKS_LAB_2F_COOLTRAINERF_STELLA
	const EVENT_OAKS_LAB_2F_SUPER_NERD_SHAWN
	const EVENT_OAKS_LAB_2F_POKEMANIAC_TANNER
	const EVENT_OAKS_LAB_2F_POKEFANM_JOSHUA
	const EVENT_OAKS_LAB_2F_POKEFANF_GEORGIA
	const EVENT_OAKS_LAB_2F_HIKER_KENNY
	const EVENT_OAKS_LAB_2F_BLACKBELT_WAI
	const EVENT_INVERSE_BEAT_BUG_CATCHER_ROB
	const EVENT_INVERSE_BEAT_SCIENTIST_TAYLOR
	const EVENT_INVERSE_BEAT_SCIENTIST_ELVIN
	const EVENT_INVERSE_BEAT_FISHER_JONAH
	const EVENT_INVERSE_BEAT_COOLTRAINERM_BLAKE
	const EVENT_INVERSE_BEAT_COOLTRAINERF_STELLA
	const EVENT_INVERSE_BEAT_SUPER_NERD_SHAWN
	const EVENT_INVERSE_BEAT_POKEMANIAC_TANNER
	const EVENT_INVERSE_BEAT_POKEFANM_JOSHUA
	const EVENT_INVERSE_BEAT_POKEFANF_GEORGIA
	const EVENT_INVERSE_BEAT_HIKER_KENNY
	const EVENT_INVERSE_BEAT_BLACKBELT_WAI
	const EVENT_STARTED_INVERSE_QUEST
	const EVENT_BUGSY_WANTS_NEUTRAL_BATTLE
	const EVENT_COMPLETED_INVERSE_QUEST
	const EVENT_CINNABAR_POKECENTER_SAILOR
	const EVENT_ROUTE_45_HIDDEN_ALOE_VERA
	const EVENT_SILVER_CAVE_ROOM_3_MAX_REPEL
	const EVENT_SILVER_CAVE_ROOM_3_FULL_RESTORE
	const EVENT_SILPH_CO_4F_X_SP_DEFEND
...
; Phone events
	...
	const EVENT_GOT_ICE_PUNCH_FROM_SANDSTORM_LADY
	const EVENT_BEVERLY_ASKED_FOR_PHONE_NUMBER
	const EVENT_GOT_THUNDERPUNCH_FROM_SANDSTORM_LADY
	const EVENT_HUEY_ASKED_FOR_PHONE_NUMBER
	const EVENT_GOT_FIRE_PUNCH_FROM_SANDSTORM_LADY
...
; More Crystal-exclusive events in Johto
	...
	const EVENT_SEAFOAM_CAVE_HIDEOUT_HIDDEN_SILVER_LEAF
	const EVENT_WADE_HAS_BERRY
	const EVENT_WADE_HAS_SWEET_BERRY
	const EVENT_WADE_HAS_CHEWY_BERRY
	...
	const EVENT_WON_HEDGER_FROM_BUG_CONTEST ; previously EVENT_GOT_GS_BALL_FROM_POKECOM_CENTER
; new to Grate Crystal
	...
	const EVENT_GATE_2F_GAMEBOY_KID
	const EVENT_GATE_2F_BUG_CATCHER
	const EVENT_GATE_2F_TWIN
	const EVENT_GATE_2F_YOUNGSTER
	const EVENT_GATE_2F_COOLTRAINER_F
	const EVENT_GATE_2F_COOLTRAINER_M
	const EVENT_GATE_2F_LASS
	const EVENT_GATE_2F_FISHER
	const EVENT_GATE_2F_TEACHER
	const EVENT_GATE_2F_POKEFAN_M
	const EVENT_GATE_2F_POKEFAN_F
	const EVENT_GATE_2F_GRAMPS
...
; Nuzlocke area catches
	...
	const EVENT_NUZLOCKE_UNDERGROUND_PATH
	...
	const EVENT_NUZLOCKE_SAFARI_ZONE
...
; Beauty
	...
	const EVENT_BEAT_SWIMMERF_JACLYN
	const EVENT_BEAT_BEAUTY_BRENDA
	const EVENT_BEAT_BEAUTY_CASSIE
	const EVENT_BEAT_POKEFANF_CAROLINE
	const EVENT_BEAT_BEAUTY_JULIA
	const EVENT_BEAT_SKIER_THERESA
...
; Kimono Girl
	const EVENT_LOST_FIRST_RIVAL_BATTLE ; new to Grate Crystal
...
; GruntM
	const EVENT_BEAT_ROCKET_GRUNTM_WELL1
	const EVENT_BEAT_ROCKET_GRUNTM_WELL2
	const EVENT_BEAT_ROCKET_GRUNTM_WELL3
	const EVENT_BEAT_ROCKET_GRUNTM_STRAY
	const EVENT_BEAT_ROCKET_GRUNTM_BASE1_1
	const EVENT_BEAT_ROCKET_GRUNTM_BASE1_2
	const EVENT_BEAT_ROCKET_GRUNTM_BASE2_1
	const EVENT_BEAT_ROCKET_GRUNTM_BASE2_2
	const EVENT_BEAT_ROCKET_GRUNTM_BASE2_3
	const EVENT_BEAT_ROCKET_GRUNTM_HYUCK1
	const EVENT_BEAT_ROCKET_GRUNTM_RADIO1
	const EVENT_BEAT_ROCKET_GRUNTM_RADIO2_1
	const EVENT_BEAT_ROCKET_GRUNTM_RADIO2_2
	const EVENT_BEAT_ROCKET_GRUNTM_SPECIALIST
	const EVENT_BEAT_ROCKET_GRUNTM_RADIO3_1
	const EVENT_BEAT_ROCKET_GRUNTM_RADIO3_2
	const EVENT_BEAT_ROCKET_GRUNTM_RADIO3_3
	const EVENT_BEAT_ROCKET_GRUNTM_BOUNCER
	const EVENT_BEAT_ROCKET_GRUNTM_UNDER1
	const EVENT_BEAT_ROCKET_GRUNTM_UNDER2
	const EVENT_BEAT_ROCKET_GRUNTM_UNDER3
	const EVENT_BEAT_ROCKET_GRUNTM_WARE1
	const EVENT_BEAT_ROCKET_GRUNTM_TRICKSTER
	const EVENT_BEAT_ROCKET_GRUNTM_HYUCK2
...
; New to Grate Crystal
	...
	const EVENT_LAV_RADIO_TOWER_TAKEOVER
	const EVENT_FOUGHT_MEWTWO
	const EVENT_WALL_OPENED_IN_SEAFOAM_CAVE
	const EVENT_VICTORY_ROAD_HIDDEN_MAX_ELIXER
	const EVENT_SEAFOAM_CAVE_DEPTHS_HIDDEN_BERRY_JUICE
	const EVENT_BEAT_GUITARIST_JULES
	const EVENT_BEAT_POKEFANF_VI
	const EVENT_BEAT_TEACHER_FRIZZLE
	const EVENT_GOT_EXP_SHARE_FROM_FRIZZLE
	const EVENT_BEAT_OFFICER_TUBBS
	const EVENT_BEAT_BURGLAR_FIZBO
	const EVENT_BEAT_SKIER_SKYE
	const EVENT_BEAT_BOARDER_SVLAD
	const EVENT_BEAT_BOARDER_EOIN
	const EVENT_BEAT_GUITARIST_CASEY
	const EVENT_BEAT_MEDIUM_CHELL
	const EVENT_BEAT_SCIENTIST_HEINZ
	const EVENT_BEAT_BIKER_T_BONE
	const EVENT_BEAT_OFFICER_DISHER
	const EVENT_BEAT_TRAINER_IGGY
	const EVENT_BEAT_TRAINER_FRED
	const EVENT_BEAT_TRAINER_CALLA
	const EVENT_BEAT_BEAUTY_MAEVE
	const EVENT_BEAT_MYSTIC_RICO
	const EVENT_BEAT_MYSTIC_OLIE
	const EVENT_BEAT_MYSTIC_GROVER
	const EVENT_BEAT_KIMONO_GIRL_KYOSHI
	const EVENT_BEAT_FIREBREATHER_IROH
	const EVENT_BEAT_SKIER_MIKAELA
	const EVENT_BEAT_BURGLAR_RAMON
	const EVENT_BEAT_OLD_MAN
	const EVENT_MOUNT_MORTAR_1F_OUTSIDE_HIDDEN_DUSK_BALL
	const EVENT_DARK_CAVE_BLACKTHORN_ENTRANCE_HIDDEN_DUSK_BALL
	...
	const EVENT_UNION_CAVE_SLOWPOKE_WELL_LINK_HIDDEN_DUSK_BALL
	const EVENT_ROUTE_22_HIDDEN_TIMER_BALL
	const EVENT_ROUTE_22_HIDDEN_RAGECANDYBAR
	const EVENT_SEAFOAM_CAVE_1F_HIDDEN_X_SP_DEFEND
	const EVENT_BEAT_JUGGLER_IRVING
	const EVENT_BEAT_GAMBLER_LUCKY
	const EVENT_GOT_COINS_FROM_GAMBLER_LUCKY
	const EVENT_BEAT_GAMBLER_HARVEY
	const EVENT_GOT_COINS_FROM_GAMBLER_HARVEY
	const EVENT_BEAT_GAMBLER_ACE
	const EVENT_GOT_COINS_FROM_GAMBLER_ACE
	const EVENT_BEAT_GAMBLER_HUCK
	const EVENT_GOT_COINS_FROM_GAMBLER_HUCK
	const EVENT_BEAT_GAMBLER_THOREAU
	const EVENT_GOT_COINS_FROM_GAMBLER_THOREAU
	const EVENT_BEAT_GAMBLER_P_T
	const EVENT_GOT_COINS_FROM_GAMBLER_P_T
	const EVENT_BEAT_GAMBLER_LIU_HAI
	const EVENT_GOT_COINS_FROM_GAMBLER_LIU_HAI
	...
	const EVENT_BEAT_COOLTRAINER_F_AZURE
	const EVENT_BEAT_SUPER_NERD_DAVE
	const EVENT_BEAT_GENTLEMAN_SYLVAIN
	const EVENT_BEAT_JUGGLER_DALTON
	const EVENT_BEAT_SCIENTIST_HIDEO
	const EVENT_BEAT_SCIENTIST_CONNOR
	const EVENT_BEAT_SCIENTIST_HEGEL
	const EVENT_BEAT_SCIENTIST_MENDEL
	const EVENT_BEAT_SCIENTIST_BEAU
	const EVENT_BEAT_SCIENTIST_TAYLOR
	const EVENT_BEAT_SCIENTIST_ELVIN
	...
	const EVENT_BLACKTHORN_GYM_STATUE
	const EVENT_BLACKTHORN_GYM_DRAGONITE
	const EVENT_BLACKTHORN_GYM_DRAGONITE_FLY
	const EVENT_BLACKTHORN_GYM_TALKED_TO_STATUE
	...
	const EVENT_BEAT_COOLTRAINERM_TEAL
	const EVENT_BEAT_BEAUTY_BERYL
	...
	const EVENT_SAFARI_ZONE_SW_MAGIKARP
	const EVENT_SAFARI_ZONE_SW_SUNFLORA
	const EVENT_SAFARI_ZONE_SW_SUDOWOODO
	const EVENT_SAFARI_ZONE_SW_BLISSEY
	const EVENT_SAFARI_ZONE_SW_MR_MIME
	const EVENT_SAFARI_ZONE_E_AERODACTYL
	const EVENT_SAFARI_ZONE_N_XATU
	...
	const EVENT_MOUNT_MOON_1F_HIDDEN_MOON_BALL
	const EVENT_MOUNT_MOON_1F_HIDDEN_STARDUST
	const EVENT_MOUNT_MOON_B1F_HIDDEN_MAX_REPEL
	const EVENT_UNION_CAVE_HIDDEN_BRICK_PIECE
	const EVENT_DARK_CAVE_VIOLET_ENTRANCE_ESCAPE_ROPE
	const EVENT_DARK_CAVE_VIOLET_ENTRANCE_DUSK_BALL
	const EVENT_DARK_CAVE_VIOLET_ENTRANCE_HIDDEN_X_ACCURACY
	const EVENT_DARK_CAVE_VIOLET_ENTRANCE_HIDDEN_X_EVADE
	const EVENT_VIOLET_CITY_BIG_MUSHROOM
	const EVENT_ROUTE_32_HIDDEN_TINYMUSHROOM
	const EVENT_RUINS_OF_ALPH_OUTSIDE_ESCAPE_ROPE
	...
	const EVENT_SAFARI_ZONE_E_ESCAPE_ROPE
	const EVENT_SAFARI_ZONE_N_X_EVADE
; Unused: next 116 (now 0) events
...
; Johto itemballs
	...
	const EVENT_DARK_CAVE_BLACKTHORN_ENTRANCE_TM_PSYCH_UP
...
; Johto people
	...
	const EVENT_CHERRYGROVE_MART_DELIBIRD ; new to Grate Crystal
...
; New to Crystal
	...
	const EVENT_PICKED_UP_BERRY_JUICE_FROM_HO_OH_ITEM_ROOM
	const EVENT_PICKED_UP_SILVER_BERRY_FROM_HO_OH_ITEM_ROOM
	const EVENT_PICKED_UP_SACRED_ASH_FROM_HO_OH_ITEM_ROOM
	const EVENT_PICKED_UP_DRAGON_FANG_FROM_HO_OH_ITEM_ROOM
	const EVENT_PICKED_UP_GOLD_BERRY_FROM_KABUTO_ITEM_ROOM
	const EVENT_PICKED_UP_TM_FALSE_SWIPE_FROM_KABUTO_ITEM_ROOM
	const EVENT_PICKED_UP_OLD_GATEAU_FROM_KABUTO_ITEM_ROOM
	const EVENT_PICKED_UP_BUGWORT_FROM_KABUTO_ITEM_ROOM
	const EVENT_PICKED_UP_TM_SELFDESTRUCT_FROM_OMANYTE_ITEM_ROOM
	...
	const EVENT_PICKED_UP_TM_U_TURN_FROM_AERODACTYL_ITEM_ROOM
	const EVENT_PICKED_UP_SOOTFOOTROOT_FROM_AERODACTYL_ITEM_ROOM
...
; New to Grate Crystal
	const EVENT_ELITE_HIDE_AND_SEEK
	const EVENT_ICE_PATH_ARTICUNO_CHAMBER_BRIGHTPOWDER
	const EVENT_ICE_PATH_ARTICUNO_CHAMBER_ARTICUNO
	const EVENT_ROCK_TUNNEL_ZAPDOS_CHAMBER_SCOPE_LENS
	const EVENT_ROCK_TUNNEL_ZAPDOS_CHAMBER_ZAPDOS
	const EVENT_SILVER_CAVE_MOLTRES_CHAMBER_SACRED_ASH
	const EVENT_SILVER_CAVE_MOLTRES_CHAMBER_MOLTRES
	const EVENT_MEW_ANTECHAMBER_VOLTORB
	const EVENT_MEW_ANTECHAMBER_TM_DARK_PULSE
	const EVENT_MEW_CHAMBER_MEW
	const EVENT_MR_POKEMONS_HOUSE_OAK_BATTLE
	const EVENT_LAV_RADIO_TOWER_1F_OFFICER
	const EVENT_LAV_RADIO_TOWER_1F_WANDERING_SUPER_NERD
	const EVENT_LAV_RADIO_TOWER_2F_MEWTWO
	const EVENT_SEAFOAM_CAVE_DEPTHS_FERALIGATR
	const EVENT_SEAFOAM_CAVE_DEPTHS_OAK
	const EVENT_SEAFOAM_CAVE_DEPTHS_PSN_OAK
	const EVENT_ROUTE_22_HOUSE_KAREN
	const EVENT_ROUTE_22_HOUSE_KOGA
	const EVENT_ROUTE_22_HOUSE_GIOVANNI
	const EVENT_ROUTE_22_HOUSE_JESSIE
	const EVENT_ROUTE_22_HOUSE_JAMES
	const EVENT_ROUTE_22_HOUSE_LANCE
	const EVENT_ROUTE_22_HOUSE_MISSINGNO
	const EVENT_ROUTE_22_HOUSE_MR_FUJI
	const EVENT_ROUTE_22_HOUSE_OFFICER_JENNY
	const EVENT_SOUL_HOUSE_MR_FUJI
	const EVENT_SILVER_CAVE_OUTSIDE_TM_NIGHT_TERROR
	const EVENT_UNION_CAVE_SLOWPOKE_WELL_LINK_PICKAXE
	const EVENT_MOUNT_MORTAR_B1F_GRAVITY_BUOY
	const EVENT_SEAFOAM_CAVE_DEPTHS_SACRED_ASH
	const EVENT_ILEX_FOREST_GOLD_LEAF
	const EVENT_BATTLE_TOWER_OUTSIDE_TM_SUBSTITUTE
	const EVENT_DARK_CAVE_2F_FOCUS_BAND
	const EVENT_SILPH_CO_2F_TRIDENT
	const EVENT_SILPH_CO_3F_CATCH_CHARM
	const EVENT_SILPH_CO_5F_SCOPE_LENS
	const EVENT_SILPH_CO_6F_TM_MOONBLAST
	const EVENT_SILPH_CO_7F_VOLTECCER
	const EVENT_SILPH_CO_8F_DUSK_BALL
	const EVENT_SAFARI_ZONE_SW_BONE
	const EVENT_SAFARI_ZONE_SW_CALCIUM
	const EVENT_ROUTE_30_TM_FAIRY_WIND
	const EVENT_ROUTE_8_HP_UP
