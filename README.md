...
- Several NPCs who allow you to view the stats of every move in the game
...
- Battle Tower edits:
  ...
  - 3 more Pokémon added to each randomized pool (Lv10, Lv20, etc...., Lv100), plus an additional 26 Pokémon for the Lv100 challenge to pull from
  - After you've beaten Red, your 7th opponent in the Lv100 challenge will be replaced by one of 26 special trainers (random each time), consisting of Gym Leaders, League members, etc.
...
- In Inverse Battles, immunities are shifted around instead of eliminated, but Normal and Ghost are still super effective on each other
...
- Olivine Gym redesigned, Lt. Surge's trashcan puzzle restored, Blaine's cave Gym expanded, smashable rocks added to Brock's Gym, a quick exit warp added to Sabrina's Gym, and Viridian Gym's old layout recreated with its Gen II Lego tiles
...
- AI_SETUP now always discourages stat-down moves if the player has either Mist or a Substitute, and always encourages stat-up moves if the player is currently in the middle of Fly or Dig and the AI's Pokémon is faster, thus avoiding a wasted turn. It also encourages both stat-up and stat-down moves if the player is using Bide, barring the aforementioned Mist and Substitute; and discourages stat-up moves during Perish Song, barring Bide, Fly, or Dig. When none of those conditions are met, it still has its vanilla behavior: 50% chance to encourage stat-up moves on the AI Pokémon's first turn, 50% chance to encourage stat-down moves on the player's Pokémon's first turn, and high chance to discourage both otherwise. However, the stat-up chance is skipped if the player's Pokémon has a type that's super effective against the AI Pokémon, and the stat-down chance is skipped if the AI's Pokémon has a type that's super effective against the player's Pokémon.

- AI_TYPES now encourages moves in the Rain Dance and Sunny Day lists if those weather conditions are up, in addition to its type effectiveness check. It also encourages Blizzard in Hail. The check for type immunities, as opposed to weaknesses and resistances, has been broken off from AI_TYPES into a new layer, AI_IMMUNITIES, which now appears alongside AI_AGGRESSIVE in addition to alongside AI_TYPES (and sometimes without either). AI_TYPES and AI_IMMUNITIES both now only check damaging moves, since many non-damaging moves ignore type immunities.
...
- Metal Coat and Dragon Scale can evolve Pokémon by leveling up while holding them, in addition to trading while holding them
