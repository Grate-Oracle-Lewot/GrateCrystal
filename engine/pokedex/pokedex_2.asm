	...
	const DEXENTRY_AREA_RODS        ; 15
	const DEXENTRY_AREA_TREES       ; 16
...
Field_Moves_List:
	db SOFTBOILED, MILK_DRINK, HEADBUTT, SWEET_SCENT, DIG, TELEPORT, CUT,
	FLY, SURF, STRENGTH, FLASH, WATERFALL, WHIRLPOOL, ROCK_SMASH
Field_Moves_Method_List:
	db 0, 0, TM01 + 1, TM01 + 11, TM01 + 27, TM01 + 57, HM01,
	HM01 + 1, HM01 + 2, HM01 + 3, HM01 + 4, HM01 + 5, HM01 + 6, HM01 + 7
...
Dex_FindFirstList:
; contest, trees(+rocks), grass, rods, water surf, surf
; grass, contest, trees(+rocks), surf, rods
	ld hl, JohtoGrassWildMons
...
Pokedex_DetailedArea_rods:
	ld a, [wPokedexStatus]; wildmon entry index, 0 we havent started yet, 1 is old-shore, 2 is good-shore, 3 is super-shore, 4 is odd-shore, 5 is old-ocean, etc
	and a
...
; given: set hl coords to baseline location SAME AS ROD, [[[10]]]
	...
	push de	
	push bc
	ld c, b ; can be 1, 2, 3, [or 4?] we wouldn't be here if we havent printed a rod
	ld a, 20
	call SimpleMultiply ; uses c as the other number
	; result in a
	ld d, 0
...
.notfound
	...
	pop hl
	pop bc
	inc c ; 0 -> 1, good rod, 1 -> 2 super rod, 2 -> 3 odd rod
	ld a, 4
	; will most likely move this check to parent function and be able to query based on rod
	cp c
	ret z ; means we've checked 4th rod, odd rod, done, no match
	jr .rod_loop
...
; Multiply hMultiplicand (3 bytes) by hMultiplier. Result in hProduct.
	...
	pop af ; 0 = old rod, 1 = good rod, 2 = super rod, 3 = odd rod
...
Rods_check_any_remaining:
	...
	cp 3 ; odd rod
	jr z, .odd
	; we are at good or super rod rn
