; See also data/battle/held_heal_status.asm

StatusHealingActions:
	;  item,         party menu action text, status
	db ANTIDOTE,     PARTYMENUTEXT_HEAL_PSN, 1 << PSN
	db BURN_HEAL,    PARTYMENUTEXT_HEAL_BRN, 1 << BRN
	db ICE_HEAL,     PARTYMENUTEXT_HEAL_FRZ, 1 << FRZ
	db AWAKENING,    PARTYMENUTEXT_HEAL_SLP, SLP
	db PARLYZ_HEAL,  PARTYMENUTEXT_HEAL_PAR, 1 << PAR
	db FULL_HEAL,    PARTYMENUTEXT_HEAL_ALL, %11111111
	db FULL_RESTORE, PARTYMENUTEXT_HEAL_ALL, %11111111
	db OLD_GATEAU,   PARTYMENUTEXT_HEAL_ALL, %11111111
	db SWEET_BERRY,  PARTYMENUTEXT_HEAL_PSN, 1 << PSN
	db CHEWY_BERRY,  PARTYMENUTEXT_HEAL_PAR, 1 << PAR
	db SPICY_BERRY,  PARTYMENUTEXT_HEAL_FRZ, 1 << FRZ
	db MINTY_BERRY,  PARTYMENUTEXT_HEAL_BRN, 1 << BRN
	db SOUR_BERRY,   PARTYMENUTEXT_HEAL_SLP, SLP
	db FRANKENBERRY, PARTYMENUTEXT_HEAL_ALL, %11111111
	db CLEAR_BELL,   PARTYMENUTEXT_HEAL_SLP, SLP
	db -1, 0, 0 ; end
